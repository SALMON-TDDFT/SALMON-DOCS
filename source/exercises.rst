...
Input files
^^^^^^^^^^^
To run the code, the input file classicEM_rt_response.inp that contains namelist variables and their values for the linear response calculation is required.
The shape file of the metallic nanosphere shape.cube is also required.
|The shape file can be generated by program 'FDTD_make_figani' in SALMON utilities:
